...
#include "Pin_TEST.h"
    ...
    #define Pin_TEST_SetP4PinDriveMode(shift, mode)  \
    do { \
        Pin_TEST_PC =   (Pin_TEST_PC & \
                                (uint32)(~(uint32)(Pin_TEST_DRIVE_MODE_IND_MASK << \
                                (Pin_TEST_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (Pin_TEST_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define Pin_TEST_SetP4PinDriveMode(shift, mode)  \
        do { \
            Pin_TEST_USBIO_CTRL_REG = (Pin_TEST_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(Pin_TEST_DRIVE_MODE_IND_MASK << \
                                    (Pin_TEST_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (Pin_TEST_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		Pin_TEST_SetP4PinDriveMode(Pin_TEST__0__SHIFT, mode);
    }
#endif
...
void Pin_TEST_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  Pin_TEST_INTCFG & (uint32)(~(uint32)position);
    Pin_TEST_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
